...
__all__ = ['ResNet2p1d', 'resnet10_2p1d', 'resnet18_2p1d', 'resnet26_2p1d',
           'resnet34_2p1d', 'resnet50_2p1d', 'resnet101_2p1d', 'resnet152_2p1d',
           'resnet200_2p1d', 'resnext50_32x4d_2p1d', 'resnext101_32x8d_2p1d',
           'wide_resnet50_2_2p1d', 'wide_resnet101_2_2p1d']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        midplanes1 = (inplanes * planes * 3 * 3 * 3) // (inplanes * 3 * 3 + planes * 3)
        ...
        midplanes2 = (planes * planes * 3 * 3 * 3) // (planes * 3 * 3 + planes * 3)
...
class ResNet2p1d(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=True,
        ...
        midplanes = (3 * self.inplanes * 3 * 7 * 7) // (3 * 7 * 7 + self.inplanes * 3)
        self.conv1_s = nn.Conv3d(inplanes, midplanes, kernel_size=(1, 7, 7),
                                 stride=(1, first_stride, first_stride),
                                 padding=(0, 3, 3), bias=False)
        self.bn1_s = norm_layer(midplanes)
        self.conv1_t = nn.Conv3d(midplanes, self.inplanes, kernel_size=(3, 1, 1),
...
def resnext50_32x4d_2p1d(**kwargs):
    kwargs['groups'] = 32
    kwargs['width_per_group'] = 4
    return ResNet2p1d(Bottleneck, [3, 4, 6, 3], **kwargs)


def resnext101_32x8d_2p1d(**kwargs):
    kwargs['groups'] = 32
    kwargs['width_per_group'] = 8
    return ResNet2p1d(Bottleneck, [3, 4, 23, 3], **kwargs)


def wide_resnet50_2_2p1d(**kwargs):
    kwargs['width_per_group'] = 64 * 2
    return ResNet2p1d(Bottleneck, [3, 4, 6, 3], **kwargs)


def wide_resnet101_2_2p1d(**kwargs):
    kwargs['width_per_group'] = 64 * 2
    return ResNet2p1d(Bottleneck, [3, 4, 23, 3], **kwargs)
