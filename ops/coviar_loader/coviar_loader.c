...
static PyObject *check_integrity(PyObject *self, PyObject *args)
{
    int ret = 0;
    ...
    if (!PyArg_ParseTuple(args, "s", &filename)) {
        fprintf(stderr, "Error: input arguments are incorrect\n");
    ...
    /* find stream information in the video */
    ...
    /* check format context (find more in av_dump_format) */
    if (fmt_ctx->duration == AV_NOPTS_VALUE ||
        fmt_ctx->start_time == AV_NOPTS_VALUE ||
        !fmt_ctx->bit_rate) {
        fprintf(stderr, "Video is corrupted\n");
        ret = -4;
        goto end;
    }
...
static PyMethodDef CoviarMethods[] = {
    ...
    {"check_integrity", check_integrity, METH_VARARGS, "Check integrity of a video."},
